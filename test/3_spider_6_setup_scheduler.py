...
def store_weather_data(conn, cursor, city, data):
    ...
    update_time = datetime.fromisoformat(
        data.get("updateTime").replace("+08:00", "+08:00")
    ).replace(tzinfo=TZ)
    ...
    for day in daily_data:
        ...
        try:
            ...
            print(
                f"{city} {fx_date} 数据写入/更新成功: {day.get('textDay')}, 影响行数: {cursor.rowcount}"
            )
...
def setup_scheduler():
    ...
    while True:
        ...
        current_time = datetime.now(TZ)
        print(f"[测试日志] 检查待执行任务: {current_time}")
        time.sleep(10)


def test_setup_scheduler():
    # 获取当前 TZ 时间，设置任务在 10 秒后触发
    now = datetime.now(TZ)
    trigger_time = (now + timedelta(seconds=10)).strftime("%H:%M:%S")

    print(f"[测试日志] 当前 PDT 时间: {now}")
    print(f"[测试日志] 设置任务在 {trigger_time} 触发 update_weather")

    # 模拟 setup_scheduler
    schedule.every().day.at(trigger_time).do(update_weather)

    # 运行 20 秒以观察任务触发
    end_time = now + timedelta(seconds=20)
    while datetime.now(TZ) < end_time:
        schedule.run_pending()
        print(f"[测试日志] 检查待执行任务: {datetime.now(TZ)}")
        time.sleep(1)
